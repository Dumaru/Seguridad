 ...
 * <p>
 * Illustrative code for the DES block cipher. Handles a single block encryption
 * or decryption, with diagnostic logging of intermediate values if required.
 * <p>
 * DES is a block cipher with an 8 byte block size. The key length is 8 bytes,
 * but only 56 bits are used as the parity bit in each byte is ignored.
 * <p>
 ...
 * <b>References:</b>
 * <ol>
 * <li><a href="mailto:schneier@counterpane.com">Bruce Schneier</a>, "Chapter 12
 * Data Encryption Standard," <cite>Applied Cryptography, 2nd edition</cite>,
 * John Wiley &amp; Sons, 1996.
 * <p>
 * <li>NIST FIPS PUB 46-2 (supercedes FIPS PUB 46-1), "Data Encryption
 * Standard", U.S. Department of Commerce, December 1993.<br>
 ...
 * <cite>J Seberry, J Pieprzyk, "Cryptography", Prentice-Hall 1989</cite>, and
 * Java <a href="http://www.systemics.com/docs/cryptix/">Cryptix</a> code.
 * <p>
 *
...
public class DES {
	...
	public static final int ROUNDS = 8, // DES has 8 rounds + output
	...
	/**
	 * DES key schedule. Stored as an array of 16 round subkeys by 8 S-box input
	 * values
	 */
	 ...
	 * <p>
	 * Rewritten as a 64 bit permutation, where 4 parity bits are permuted to DES
	 * bits 29-32 in each 32 bit int, and subsequently ignored.
	 */
	private static final byte[] PC1 = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27,
			19, 11, 3, 60, 52, 44, 36, 8, 16, 24, 32, // <- these are parity bits to ignore
	 ...
	 * <p>
	 * nb: DES bits (16) select row in each table DES bits (2345) select column in
	 * row
	 */
	 ...
	 *
	 * <p>
	 * DES encryption is IP, 16 rounds using f, FP with subkeys in order
	 *
	...
	public byte[] encrypt(byte[] plain) {
		...
		if (traceLevel > 0)
			traceInfo = "encryptDES(" + Util.toHEX1(plain) + ")";
		...
		String[] rBinaries = new String[16];
		rBinaries[0] = Integer.toBinaryString(data[1]);
		for (int i = 0; i < 16; i++) { /* Compute 16 rounds on data */
			if (traceLevel > 1)
				traceInfo += "\n  Rnd" + (i + 1) + "\tf(R" + i + "=" + Util.toHEX1(data[1]) + ", SK" + (i + 1) + "="
						+ Util.toHEX(subkeys[i]) + ") ";
			
			t = f(data[1], subkeys[i]); // Compute round fn
			if (traceLevel > 1)
			{	
				traceInfo += "= " + Util.toHEX1(t);
				traceInfo += " R"+i+" = "+Integer.toBinaryString(data[1]);
				rBinaries[i] = Integer.toBinaryString(data[1]);
				
			
				//traceInfo += " Bits cambiados " + Util.bitsChangedAmount(rBinaries[(i == 0?0:i-1)], rBinaries[i]);
		...
		if (traceLevel > 1)
			traceInfo += "\n";
		if (traceLevel > 0)
			traceInfo += " returns " + Util.toHEX1(res) + "\n";
		return (res);
	}
	 ...
	 *
	 * <p>
	 * DES decryption is IP, 16 rounds using f, FP with subkeys in reverse
	 *
	...
	public byte[] decrypt(byte[] cipher) {
		...
		if (traceLevel > 0)
			traceInfo = "decryptDES(" + Util.toHEX1(cipher) + ")";
		...
		for (int i = 0; i < 16; i++) { /* Compute 16 rounds on data */
			if (traceLevel > 1)
				traceInfo += "\n  Rnd" + (i + 1) + "\tf(R" + i + "=" + Util.toHEX1(data[1]) + ", SK" + (16 - i) + "="
						+ Util.toHEX(subkeys[15 - i]) + ") ";
	...
	// ......................................................................
	/**
	 * takes the 64-bit key to use for subsequent en/decryptions, and expands it
	 * into the DES subkey schedules.
	 *
	 * <p>
	 * The key is passed as a 64-bit value, of which the 56 non parity bits are
	 * used. The parity bits are in DES bits 8, 16, 24, 32, 40, 48, 56, 64 (nb:
	 * these do NOT correspond to the parity bits in ascii chars when packed in the
	 * usual way). The function performs the key scheduling calculation, saving the
	 * resulting sixteen 48-bit sub-keys for use in subsequent encryption/decryption
	 * calculations. These 48-bit values are saved as eight 6-bit values, as
	 * detailed previously.
	 *
	 * <p>
	 * The key scheduling calculation involves permuting the input key by PC1 which
	 * selects 56-bits dividing the 56-bit value into two halves C, D sixteen times
	 * rotates each half left by 1 or 2 places according to schedule in keyrot
	 * concatenates the two 28-bit values, and permutes with PC2 which selects
	...
	public void setKey(byte[] key) {
		...
		if (traceLevel > 0)
			traceInfo = "setKey(" + Util.toHEX1(key) + ")\n";
		...
		if (traceLevel > 3)
			traceInfo += "  PC1(Key)=" + Util.toHEX(cd) + "\n";
		...
		for (int i = 0; i < 16; i++) { // gen subkeys for all 16 rounds
			...
			// create trace info if needed
			if (traceLevel > 3)
				traceInfo += "  KeyRnd" + (i + 1) + "\tCD=" + Util.toHEX(cd) + "\tPC2(CD)=" + Util.toHEX(subkeys[i])
						+ "\n";
		}
	}
	 ...
	 * <p>
	 * Its output is a complex function of both input data and sub-key The input
	 * data R(i-1) is expanded to 48-bits via expansion fn E, is XOR'd with the
	 * subkey K(i), substituted into the S-boxes, and finally permuted by P. ie the
	 * calculation is: A = E(R(i-1)) XOR K(i) (a 48-bit value) B = S(A) (a 32-bit
	 * value) f = P(B) (a 32-bit value)
	 *
	 * nb: the 48-bit values are stored as eight 6-bit values. In each byte, the
	 * bits are numbered [x x 1 2 3 4 5 6] Overall the bit numbering is: [x x 1 2 3
	 * 4 5 6] [x x 7 8 9 10 11 12] ... [x x 43 44 45 46 47 48] nb: the 6-bit S-box
	 * input value [x x 1 2 3 4 5 6] is interpreted as: bits [1 6] select a row
	 * within each box, bits [2 3 4 5] then select a column within that row hence
	 * the input value is reordered to [x x 1 6 2 3 4 5] before indexing into the
	 * S-box tables.
	 * <p>
	 * There are 14 main events in each of 8 rounds of DES using 6 sub-keys, plus an
	 * output transformation using 4 subkeys.
	 *
	...
	/**
	 * compute the PC2 key round permutation. takes the two key halves in CD,
	 * permutes them according to PC2, and generates a 48-bit output value stored as
	 * eight 6-bit (byte) values
	 *
	 ...
	 * <p>
	 * Where n is the two 24-bit halves used in the key scheduling, and bits are
	 * numbered [1 2 ... 27 28 x x x x] [29 30 ... 56 x x x x]<br>
	 * in ints n[0] n[1]<br>
	...
	/**
	 * rotates int value b (where b is a 28-bit key half) left by pos bits, (0 <=
	 * pos <= 28).
	 *
	 ...
	 * <p>
	 * Result is stored as eight 6-bit values in a byte array, with bits numbered as
	 * specified previously.
	 * <p>
	 * Due to the regular nature of the expansion matrix E, this function implements
	 * it directly for efficiency reasons It takes each 4-bit nybble of the input
	 * word, and concatenates it with the adjacent bit on either side to form a
	 * 6-bit value. (nb: bit 32 is assumed to be adjacent to bit 1)
	 */
	private byte[] expand(int R) {
		...
		t = (R << 5) | (R >>> 27);
		a[0] = (byte) (t & MASK6); // bits 32 1 2 3 4 5
		t = R >>> 23;
		a[1] = (byte) (t & MASK6); // bits 4 5 6 7 8 9
		t = R >>> 19;
		a[2] = (byte) (t & MASK6); // bits 8 9 10 11 12 13
		t = R >>> 15;
		a[3] = (byte) (t & MASK6); // bits 12 13 14 15 16 17
		t = R >>> 11;
		a[4] = (byte) (t & MASK6); // bits 16 17 18 19 20 21
		t = R >>> 07;
		a[5] = (byte) (t & MASK6); // bits 20 21 22 23 24 25
		t = R >>> 03;
		a[6] = (byte) (t & MASK6); // bits 24 25 26 27 28 29
		t = (R << 1) | (R >>> 31);
		a[7] = (byte) (t & MASK6); // bits 28 29 30 31 32 1
		return (a);
	}

	/**
	 * general permutation of a 32-bit input block to a 32-bit output block, under
	 * the control of a permutation array perm. Each element of perm specifies which
	 * input bit is to be permuted to the output bit with the same index as the
	 * array element.
	 * <p>
	 * nb: to set bits in the output word, as mask with a single 1 in it is used. On
	 * each step, the 1 is shifted into the next location
	 */
	...
	/**
	 * general permutation of a 64-bit input block to a 64-bit output block, under
	 * the control of a permutation array perm.
	 * <p>
	 * Each element of perm specifies which input bit is to be permuted to the
	 * output bit with the same index as the array element.
	 * <p>
	 * nb: to set bits in the output word, a mask with a single 1 in it is used. On
	 * each step, the 1 is shifted into the next location
	 */
	...
	public static void main(String[] args) {
		...
		self_test("5B5A57676A56676E", "675A69675E5A6B5A", "974AFFBF86022D1F", lev);
	}
}
